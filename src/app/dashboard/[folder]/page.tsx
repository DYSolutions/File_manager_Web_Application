...
export default function Dashboard() {
  ...
  const fetchFolderData = async () => {
    try {
      ...
      const q = query(collection(db, `Folders/${id}/subFolders`), where("createdBy", "==", session?.user?.email));
    ...
    <div className="p-4 min-h-screen w-full flex flex-col items-center">
      <div className="flex flex-col w-[95%] bg-white h-[auto] rounded-2xl mt-[30px] p-6 gap-4">
        <div className="flex flex-row justify-between items-center px-2">
          <h2 className="text-[15px] text-black font-semibold">{name}</h2>
          <a href="">view more</a>
        </div>
        <div className="grid grid-cols-5 w-auto gap-0">
          {folders.map((folder) => (
            <div key={folder.id} className="p-4 border border-[#74717136] rounded-md h-[120px] w-[120px] flex flex-col items-center justify-center hover:scale-105 transition-all cursor-pointer"><Image src="/folder.png" height={50} width={50} alt="folder" />{folder.name}</div>
          ))}
        </div>
      </div>
